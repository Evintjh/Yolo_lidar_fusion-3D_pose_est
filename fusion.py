...
def lidar_camera_fusion(pts_3D, pts_2D, frame, seg_mask, obj_class, lidar2cam, erosion_factor, depth_factor, PCA):
    ...
    #cv2.fillPoly(mask, [seg_mask.astype(np.int32)], color=1)

    # Convert 2D points to integer coordinates
    pts_2D_int = pts_2D.astype(np.int32)
    print(f'fusion 3d points: {len(pts_3D)}')
    print(f'fusion 2d points: {len(pts_2D_int)}')    
    # Filter points using the mask
    inside_mask_indices = mask[pts_2D_int[:, 1], pts_2D_int[:, 0]] == 1
    #inside_mask_indices = []
    #poly = eroded_seg.astype(np.int32)
    #for p in pts_2D_int:
    #    dist = cv2.pointPolygonTest(poly, (int(p[0]), int(p[1])), True)
    #    inside_mask_indices.append(dist >= -5)
    inside_mask_indices = np.array(inside_mask_indices)
    # Select corresponding 3D points
    all_points_of_object = pts_3D[inside_mask_indices]
    """ ------------------------------------------"""
    print(f'filtered number of points: {len(all_points_of_object)}')
