...
def filter_points_with_depth_clustering(points_of_object, eps=0.5, depth_factor=20, print_info=False):
    ...
    cluster_counts_dict.pop(-1, None)

    if not cluster_counts_dict:
        return points_of_object  # fallback

    best_cluster = max(cluster_counts_dict, key=cluster_counts_dict.get)
    print(f'cluster info: best_cluster: {best_cluster}, unique_clusters: {unique_clusters}, cluster_counts: {cluster_counts}')
    print(f'best cluster avg depth: {np.mean(np.sqrt(points_of_object[clusters == best_cluster][:, 0]**2 + points_of_object[clusters == best_cluster][:, 1]**2))}')
    ### Return all points from that cluster
    return points_of_object[clusters == best_cluster]
...
# uses different convert_3D_to_2D function -> convert_3D_to_camera_coords
